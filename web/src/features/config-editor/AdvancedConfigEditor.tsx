...
export default function AdvancedConfigEditor() {
  ...
  return (
    <div className="flex h-full">
      {/* Sidebar */}
      <div className={`border-r bg-background_alt ${mobileMenuOpen ? 'fixed inset-0 z-40 flex' : 'hidden'} md:flex md:relative md:w-56 shrink-0 flex-col h-full max-h-full w-full`}>
        <div className="flex items-center justify-between p-2 md:hidden shrink-0 border-b">
          <div className="font-semibold">{t('config.title', { defaultValue: 'Configuration' })}</div>
          <button className="border rounded-md px-2 py-1" onClick={() => setMobileMenuOpen(false)}>Close</button>
        </div>
        <nav className="p-2 space-y-1 overflow-y-auto flex-1">
          {sections.map((s) => (
            <button
              key={s.id}
              className={`w-full text-left px-2 py-1 rounded-md ${activeSection === s.id ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}
              onClick={() => {
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <div className="flex items-center gap-2 p-2 border-b shrink-0 flex-wrap">
          <button className="md:hidden border rounded-md px-2 py-1 text-sm" onClick={() => setMobileMenuOpen((v) => !v)}>
            ☰
          </button>
          <div className="flex items-center gap-2">
            <button
              className={`border rounded-md px-2 py-1 text-sm ${activeTab === 'form' ? 'bg-muted' : ''}`}
              onClick={() => setActiveTab('form')}
            >
              Form
            </button>
            <button
              className={`border rounded-md px-2 py-1 text-sm ${activeTab === 'yaml' ? 'bg-muted' : ''}`}
          ...
          <div className="ml-auto flex items-center gap-2 flex-wrap">
            ...
            {guard.warnings.length > 0 && (
              <div className="text-xs text-amber-600 whitespace-nowrap">{guard.warnings.length} warn</div>
            )}
            {guard.errors.length > 0 && (
              <div className="text-xs text-destructive whitespace-nowrap">{guard.errors.length} err</div>
            )}
            <button
        ...
        {/* Messages */}
        {guard.errors.length > 0 && (
          <div className="m-2 p-2 border border-destructive rounded-md text-destructive text-sm">
        ...
        {guard.warnings.length > 0 && (
          <div className="m-2 p-2 border rounded-md text-amber-700 border-amber-300 text-sm bg-amber-50">
